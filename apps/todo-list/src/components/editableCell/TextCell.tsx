...
export const TextCell: FC<BaseCellProps<string>> = ({
    hovered, selected, onHover, onClick, onEdit, onCommit, value, widthInPx
}) => {
    ...
    return isEditing 
    ? <input value={state.draft} ref={inputRef}
        onChange={(e) => {
            setState({ mode: 'edit', draft: e.target.value })
        }}
        onKeyDown={handleKeyboardInput}
        onBlur={handleBlur}
        style={{ width: widthInPx, flex: 'none', boxSizing: 'border-box', border: 'none', padding: 'none'}}></input> 
    : <div ref={inputRef} tabIndex={0} onClick={handleClick} onMouseOver={handleHover}
